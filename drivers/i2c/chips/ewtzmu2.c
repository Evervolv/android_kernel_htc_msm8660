...
static int EWTZMU2_Report_Value(void)
{
	...
	input_report_abs(data->input_dev_compass, ABS_RUDDER, ewtzmumid_data.status);/* status of orientation sensor */
	...
	input_report_abs(data->input_dev_compass, ABS_Y, ewtzmumid_data.na.y);/* y-axis raw acceleration */
	input_report_abs(data->input_dev_compass, ABS_Z, ewtzmumid_data.na.z);/* z-axis raw acceleration */
	report_enable = EW_REPORT_EN_COMPASS;
	}
