...
struct file_operations thp_fops = {
	...
	, .poll		  =	thp_poll
#if 0
	, .ioctl 	  =	thp_ioctl	/* According to http://andmasterofnone.blogspot.com/2011/09/fileoperations-structure-changes.html,
							 * The correct thing to do is to use the "unlocked_ioctl" element instead of "ioctl".
							 */
	, .unlocked_ioctl =	thp_ioctl  /* <---- Need to modified codes */
#endif
};
...
int init_thp_devfile(void)
{
	...
	if (IS_ERR(thp_class)) {
		printk(KERN_ERR "class_create error(0x%x)\n", (unsigned int)(thp_class));
...
/* #if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27) */
	...
	if (IS_ERR(thp_device))	{
		printk(KERN_ERR "device_create error(0x%x)\n", (unsigned int)(thp_device));
