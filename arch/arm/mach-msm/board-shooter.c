...
static struct isa1200_platform_data isa1200_1_pdata = {
	...
	.hap_en_gpio = PM8058_GPIO_PM_TO_SYS(shooter_HAP_ENABLE),
	.max_timeout = 15000,
...
static struct htc_headset_pmic_platform_data htc_headset_pmic_data = {
	...
	.key_irq		= 0,
	.key_enable_gpio	= PM8058_GPIO_PM_TO_SYS(shooter_AUD_REMO_EN),
	.adc_mic_bias		= {0, 0},
...
static int pm8058_gpios_init(void)
{
	...
	struct pm8058_gpio_cfg gpio_cfgs[] = {
		...
		{
			PM8058_GPIO_PM_TO_SYS(shooter_PLS_INT),
			{
				.direction		= PM_GPIO_DIR_IN,
...
static struct tpa2051d3_platform_data tpa2051d3_pdata = {
	.gpio_tpa2051_spk_en = SHOOTER_AUD_HANDSET_ENO,
...
static unsigned int msm8x60_sdcc_slot_status(struct device *dev)
{
	...
	status = gpio_request(PM8058_GPIO_PM_TO_SYS(shooter_SDC3_DET)
				, "SD_HW_Detect");
	if (status) {
		pr_err("%s:Failed to request GPIO %d\n", __func__,
				PM8058_GPIO_PM_TO_SYS(shooter_SDC3_DET));
		...
		if (!status)
			status = !(gpio_get_value_cansleep(
				PM8058_GPIO_PM_TO_SYS(shooter_SDC3_DET)));
		gpio_free(PM8058_GPIO_PM_TO_SYS(shooter_SDC3_DET));
	}
...
static struct msm_board_data shooter_board_data __initdata = {
	.gpiomux_cfgs = msm8x60_shooter_gpiomux_cfgs,
};

void shooter_add_usb_devices(void)
{
...
{
	msm8x60_init(&shooter_board_data);
	printk(KERN_INFO "%s revision=%d engineerid=%d\n", __func__, system_rev, engineerid);
}

static void __init shooter_charm_init_early(void)
{
